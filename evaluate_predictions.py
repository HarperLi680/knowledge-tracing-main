...
def load_predictions_from_json(json_file):
    """Load predictions from JSON, normalize model names, and convert to numpy arrays."""
    with open(json_file, 'r') as f:
        predictions = json.load(f)
    
    # Convert lists back to numpy arrays and ensure fold keys are integers
    converted_predictions = {}
    for raw_model in predictions:
        model = normalize_model_name(raw_model)
        if model not in converted_predictions:
            converted_predictions[model] = {}
        for fold_str, pred_list in predictions[raw_model].items():
...
def create_combined_csv(
    ...
    for fold in range(num_folds):
        ...
        for model_name in EXPECTED_MODELS:
            if model_name not in all_predictions or fold not in all_predictions[model_name]:
                print(f"Fold {fold}, {model_name}: predictions missing, filling NaN")
                fold_data[f'{model_name}_prediction'] = np.nan
                continue
...
def main():
    ...
    loaded_models = list(all_predictions.keys())
    print(f"Loaded predictions for models: {loaded_models}")
    missing_models = [m for m in EXPECTED_MODELS if m not in all_predictions]
    if missing_models:
        print(f"Missing model predictions in JSON: {missing_models}")
