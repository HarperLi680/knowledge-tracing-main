...
from Utils.utils_DSAKT import getdata, dataloader, NoamOpt
...
def train_DSAKT(lr:float, window_size:int, dim:int, dropout:float, heads:int, batch_size:int, epochs:int, train_path:str, valid_path:str):
    
    
    train_data,N_train,E_train,unit_list_train = getdata(window_size=window_size, path=train_path, model_type='sakt')
    valid_data, N_val, E_val, unit_list_val = getdata(window_size=window_size, path=valid_path, model_type='sakt');
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            acc = torch.eq(pred, cort).float().mean().item();
...
def train_predict_DSAKT(train_path,valid_path,test_path,lr,window_size,dim,dropout,heads):
    batch_size = 128;
    epochs = 100;

    model = train_DSAKT(lr,window_size,dim,dropout,heads,batch_size,epochs,train_path=train_path,valid_path=valid_path)

    test_data,N_test,E_test,unit_list_test = getdata(window_size=window_size, path=test_path, model_type='sakt')
    with torch.no_grad():
        predict = model(test_data[0].to(device), test_data[1].to(device)).squeeze(-1).to("cpu");
        correctness = test_data[2];
        
        pred = [];
        cort = [];
        for i in range(N_test):
            pred.extend(predict[i][0:unit_list_test[i]].cpu().numpy().tolist());
            cort.extend(correctness[i][0:unit_list_test[i]].numpy().tolist());
...
if __name__ =="__main__":
    ...
    parser.add_argument("-t", "--train_data", default='dataset/assist2009_DSAKT/assist2009_DSAKT_train1.csv');
    parser.add_argument("-v", "--valid_data", default='dataset/assist2009_DSAKT/assist2009_DSAKT_valid1.csv');
    parser.add_argument("-te", "--test_data", default='dataset/assist2009_DSAKT/assist2009_DSAKT_test1.csv');
    ...
    train_predict_DSAKT(window_size=window_size,
                dim=dim, 
                heads=heads, 
                dropout=dropout, 
                lr=lr, 
