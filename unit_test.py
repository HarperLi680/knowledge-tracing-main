...
def test_train_predict_LKT():
    files = sorted([f for f in os.listdir(TABULAR_DATA) if f.endswith(".csv")])

    preds, labels = train_predict_LKT(
        train_data=[os.path.join(TABULAR_DATA, files[0])],
        test_data=os.path.join(TABULAR_DATA, files[1]),

        # Keep the unit test lightweight.
    ...
    assert len(preds) == len(labels), "LKT: Prediction-label length mismatch"
...
if __name__ == "__main__":
    ...
    test_train_predict_LKT()
    print("LKT test passed!")
