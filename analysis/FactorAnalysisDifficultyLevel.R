# =========================================================
# Exploratory factor analysis by skill-level difficulty
# Quartile version: group skills by quartiles of the average
# correctness rate, then run EFA separately for each quartile.
#
# Difficulty indicator:
#   skill_accuracy = mean(correct) within each skill
#   lower accuracy = harder skill
#   higher accuracy = easier skill
#
# This script:
# 1. Loads the combined dataset
# 2. Computes avg correctness per skill
# 3. Splits skills into 4 difficulty quartiles
...
# 7. Computes AUC by difficulty quartile
# =========================================================
...
# Remove rows with missing user, skill, or correct
dat_raw <- dat_raw %>%
  filter(!is.na(user), !is.na(skill), !is.na(correct))

# ---------------------------------------------------------
# 4. Compute skill-level difficulty indicator
# ---------------------------------------------------------
# skill_accuracy = average correctness for each skill
# lower skill_accuracy = harder skill
# higher skill_accuracy = easier skill
# ---------------------------------------------------------

skill_summary <- dat_raw %>%
  group_by(skill) %>%
  summarise(
    skill_accuracy = mean(correct, na.rm = TRUE),
    skill_difficulty = 1 - skill_accuracy,
    n_rows = n(),
    n_students = n_distinct(user),
    .groups = "drop"
  )

# ---------------------------------------------------------
# 5. Create quartile-based difficulty groups
# ---------------------------------------------------------
# Q1 = hardest skills (lowest accuracy)
# Q4 = easiest skills (highest accuracy)
# ---------------------------------------------------------

skill_summary <- skill_summary %>%
  arrange(skill_accuracy) %>%
  mutate(
    difficulty_group_num = ntile(skill_accuracy, 4),
    difficulty_group = recode(
      as.character(difficulty_group_num),
      "1" = "Q1_hardest_skills",
      "2" = "Q2_harder_skills",
      "3" = "Q3_easier_skills",
      "4" = "Q4_easiest_skills"
...
# Save skill summary
dir.create("efa_by_skill_difficulty_quartile", recursive = TRUE, showWarnings = FALSE)
write.csv(
  skill_summary,
  "efa_by_skill_difficulty_quartile/skill_difficulty_summary.csv",
  row.names = FALSE
)
...
run_efa_for_group <- function(dat_subset,
                              ...
                              cols,
                              nfactors = 3,
                              output_base = "efa_by_skill_difficulty_quartile") {
...
# ---------------------------------------------------------
# 7. Run EFAs by difficulty quartile group
# ---------------------------------------------------------
...
master_summary <- dat_grouped %>%
  ...
  summarise(
    ...
    mean_skill_accuracy = mean(skill_accuracy, na.rm = TRUE),
    min_skill_accuracy = min(skill_accuracy, na.rm = TRUE),
    max_skill_accuracy = max(skill_accuracy, na.rm = TRUE),
    mean_skill_difficulty = mean(skill_difficulty, na.rm = TRUE),
    .groups = "drop"
  )

write.csv(
  master_summary,
  "efa_by_skill_difficulty_quartile/master_group_summary.csv",
  row.names = FALSE
)

cat("\nAll difficulty-quartile EFAs completed.\n")

# =========================================================
# 9. Align and visualize loadings across difficulty quartiles
# =========================================================
...
base_dir <- "efa_by_skill_difficulty_quartile"
...
aligned_loadings <- aligned_loadings %>%
  ...
  mutate(group_clean = recode(
    group_label,
    "Q1_hardest_skills" = "Q1: Hardest skills",
    "Q2_harder_skills" = "Q2: Harder skills",
    "Q3_easier_skills" = "Q3: Easier skills",
    "Q4_easiest_skills" = "Q4: Easiest skills"
  )) %>%
...
p_lines <- ggplot(
  ...
  labs(
    title = paste("Factor loadings by skill difficulty quartile -", str_to_title(loading_type)),
    x = "Skill difficulty level (quartiles)",
    y = "Aligned loading",
    color = "Model"
  )
...
p_heatmap <- ggplot(
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = paste("Heatmap of factor loadings by skill difficulty quartile -", str_to_title(loading_type)),
    x = "Skill difficulty level (quartiles)",
    y = "Model",
    fill = "Loading"
  )
...
# ---------------------------------------------------------
# 10. Compute AUCs by difficulty quartile group
# ---------------------------------------------------------

auc_results <- list()

for (g in unique(dat_grouped$difficulty_group)) {
...
p_auc_lines <- ggplot(
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = "AUC across skill difficulty levels",
    x = "Skill difficulty level (quartiles)",
    y = "AUC",
    color = "Model"
  )
...
ggsave(
  "efa_by_skill_difficulty_quartile/auc_by_quartile_lines.png",
  p_auc_lines,
  width = 10,
  height = 6,
  dpi = 300
